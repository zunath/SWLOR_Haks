...
Run from the HAK repository, using the parent SWLOR_NWN checkout for Module data:
  python tools/AuditSoundCompression.py --repo-root .. --write-exclusions audio-exclusions.json
  python tools/AuditSoundCompression.py --repo-root .. --check tools/SoundCompressionManifest.json

For a standalone HAK checkout, set --repo-root to the SWLOR_NWN repository path.
The audit fails if the parent Module data or required HAK tables are unavailable.
...
HAK_ROOT = pathlib.Path(__file__).resolve().parents[1]
LOOP_COLUMNS = {
...
def main(argv=None):
    ...
    parser.add_argument("--repo-root", type=pathlib.Path,
                        help="SWLOR_NWN repository root (default: parent of the HAK checkout)")
    parser.add_argument("--hak-root", type=pathlib.Path, default=HAK_ROOT,
                        help="HAK source root (default: this script's repository)")
    ...
    try:
        hak_root = args.hak_root.resolve(strict=True)
        repo_root = args.repo_root.resolve(strict=True) if args.repo_root else hak_root.parent
        evidence = collect_exclusions(repo_root, hak_root)
        ...
        if args.check:
            violations = check_manifest(read_json(args.check), evidence, hak_root)
            if violations:
