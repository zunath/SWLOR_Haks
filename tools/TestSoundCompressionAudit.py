...
class SoundCompressionAuditTests(unittest.TestCase):
    def test_relocated_cli_defaults_to_script_checkout_not_current_directory(self):
        with tempfile.TemporaryDirectory() as temporary:
            repo_root = pathlib.Path(temporary)
            hak_root = repo_root / "SWLOR_Haks"
            hak_root.mkdir()
            output = repo_root / "exclusions.json"
            with mock.patch.object(audit, "HAK_ROOT", hak_root), \
                    mock.patch.object(audit, "collect_exclusions", return_value={}) as collect, \
                    contextlib.redirect_stdout(io.StringIO()):
                self.assertEqual(audit.main(["--write-exclusions", str(output)]), 0)
            collect.assert_called_once_with(repo_root.resolve(), hak_root.resolve())

    def test_standalone_cli_requires_accessible_parent_module_and_accepts_explicit_repo(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = pathlib.Path(temporary)
            hak_root = root / "standalone_haks"
            tables = hak_root / "sw_2da"
            ...
            with contextlib.redirect_stderr(io.StringIO()) as errors:
                self.assertEqual(audit.main(["--hak-root", str(hak_root), "--write-exclusions", str(output)]), 1)
            self.assertIn("cannot audit loops", errors.getvalue())
            self.assertFalse(output.exists())

            repo_root = root / "separate_parent"
            for folder in ("git", "uts"):
                directory = repo_root / "Module" / folder
                directory.mkdir(parents=True)
                (directory / f"sound.{folder}.json").write_text(json.dumps(sound(folder)), encoding="utf-8")
            with contextlib.redirect_stdout(io.StringIO()):
                self.assertEqual(audit.main(["--hak-root", str(hak_root), "--repo-root", str(repo_root),
                                             "--write-exclusions", str(output)]), 0)
            self.assertEqual(set(audit.read_json(output)["excluded_resrefs"]), {"git", "uts"})
