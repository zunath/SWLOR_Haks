...
echo "Moving DDS texture files..."
total_dds=0
find swlor2_dds -name "*.dds" | grep -E "_(belt|bicepl|bicepL|bicepr|bicepR|chest|cloak|footl|footL|footr|footR|forel|foreL|forer|foreR|handl|handL|handr|handR|head|legl|legL|legr|legR|neck|pelvis|robe|shinl|shinL|shinr|shinR|shol|shoL|shor|shoR)" | while IFS= read -r file; do
...
echo "Moving TGA texture files..."
total_tga=0
find swlor2_tga -name "*.tga" | grep -E "_(belt|bicepl|bicepL|bicepr|bicepR|chest|cloak|footl|footL|footr|footR|forel|foreL|forer|foreR|handl|handL|handr|handR|head|legl|legL|legr|legR|neck|pelvis|robe|shinl|shinL|shinr|shinR|shol|shoL|shor|shoR)" | while IFS= read -r file; do
    target_dir=$(get_target_dir "$file")
    if [[ -n "$target_dir" ]]; then
        if [[ -d "$target_dir" ]]; then
            echo "Moving $(basename "$file") to $target_dir"
...
echo "Texture file movement complete!"
echo "DDS files moved: $total_dds"
echo "TGA files moved: $total_tga"